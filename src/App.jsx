...
import writingAnimation from './assets/harrypotter.json';
  ...
  return (
    <ImageList cols={imagesLength}>
      {rowImages
        ? rowImages.map((item) => {
            return (
              <ImageListItem key={item.set + item.index}>
                <img
                  ...
                  style={{
                    opacity:
                      chosen && chosen[item.set] && chosen[item.set][item.index] === 1 ? 0.3 : 1,
                  }}
                  onClick={(e) => {
                    handleChosen(item.set, item.index);
                  }}
                />
              </ImageListItem>
            );
...
function App() {
  ...
  return (
    <Box className='App'>
      <Grid container spacing={{ xs: 0, sm: 2, md: 2 }} alignItems='center' justifyContent='center'>
        <Grid
          paddingX={2}
          container
          alignItems='center'
          ...
          <Typography align='center' variant='h2' gutterBottom sx={{ marginBottom: 2 }}>
            Storybook
          </Typography>

          <Typography align='center' sx={{ marginBottom: 5 }} variant='h4' gutterBottom>
          ...
          <div style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <Recorder body={body} setBody={setBody} />
          </div>
      ...
      {video ? (
        <a target='_blank' href={video}>
          Click here in several minutes to see the video
        </a>
      ) : null}
    </Box>
  );
